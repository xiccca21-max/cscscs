...
Порт **5432 (Postgres) наружу не открывать** — в `docker-compose.yml` он не пробрасывается на хост. К БД с сервера: `docker compose exec db psql -U postgres`. Если фаервол у провайдера в панели — открыть только то, что нужно (часто 22 и 80/443), не 5432.
