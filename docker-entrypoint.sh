...
if CI=1 prisma db push 2>&1; then
  echo "[docker-entrypoint] schema ok"
else
  echo "[docker-entrypoint] WARNING: prisma db push failed — starting server anyway. Run manually: docker compose exec app prisma db push" >&2
fi
exec node server.js
